...
public class FindGoodDaysToRobTheBank {
    ...
    class Solution {
        public List<Integer> goodDaysToRobBank(int[] security, int time) {
            ...
            int nonIncreasingCount = 0, leftOne = -1;
            for (int i = 0; i < len; i++) {
                nonIncreasingCount = security[i] > leftOne ? 0 : nonIncreasingCount + 1;
                left[i] = nonIncreasingCount;
                leftOne = security[i];
            }
            int nonDecreasingCount = 0, rightOne = -1;
            for (int i = len - 1; i >= 0; i--) {
                nonDecreasingCount = security[i] > rightOne ? 0 : nonDecreasingCount + 1;
                right[i] = nonDecreasingCount;
                rightOne = security[i];
            }
