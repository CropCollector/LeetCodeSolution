...
public class MaximumNumberOfEatenApples {
    public static void main(String[] args) {
        ...
        System.out.println(solution.eatenApples(new int[]{3,1,1,0,0,2}, new int[]{3,1,1,0,0,2}));
    }

    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public int eatenApples(int[] apples, int[] days) {
            int ans = 0, len = apples.length;
            // arr[2] arr[0] 苹果数 arr[1] 过期时间
            PriorityQueue<int[]> pq = new PriorityQueue<>(Comparator.comparingInt(arr -> arr[1]));
            int i;
            for (i = 0; i < len || !pq.isEmpty(); i++) {
                if (i < len && apples[i] != 0) {
